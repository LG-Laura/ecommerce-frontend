...
function loadCart() {
    ...
    if (userId) {
        fetch(`https://ecommerce-backend-vevb.onrender.com/api/users/${userId}/cart`, {
            method: 'GET',
            headers: {
...
async function addToCart(productId) {
    ...
    const userId = localStorage.getItem('userId');
    try {
        const productResponse = await fetch(`https://ecommerce-backend-vevb.onrender.com/api/products/${productId}`);
...
function removeFromCart(productId) {
    ...
    const userId = localStorage.getItem('userId');
    if (userId) {
        fetch(`https://ecommerce-backend-vevb.onrender.com/api/users/${userId}/cart/${productId}`, {
            method: 'DELETE',
...
function proceedToPayment() {
    ...
    if (cart.length > 0) {
        fetch('https://ecommerce-backend-vevb.onrender.com/api/payment', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
